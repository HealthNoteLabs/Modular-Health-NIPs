...
On real numbers, as of June 2026: RUNSTR has recorded **7,705 verified workouts from 970 unique users**, and its zapper has sent **1,886 successful Bitcoin micropayments totaling 127,963 sats** to **334 distinct recipients**, with roughly **22% of reward volume routed to charities** by users. These are early-stage figures for a bootstrapped, fully open-source project (MIT-licensed, in active development since early 2025 across more than 1,300 commits), and the honest position of this paper is to report them as they are rather than inflate them. The gap between 970 users and 334 paid recipients is worth stating plainly: the project started in February 2025 and logged its first workout data in April 2025, but instant rewards did not launch until January 2026, so most workouts were recorded before any payout was possible — and not every user has set up a Lightning destination to receive sats. The point of the numbers is not their size. It is that they are real: real workouts published as user-owned events, real Bitcoin moving peer-to-peer through an automated zapper, on a system that is live and running today. What can be stated without qualification is the architectural fact underneath them: RUNSTR publishes real, user-owned, encrypted health and workout events to Nostr, pays real Bitcoin, and does it on the open NIP-101h and NIP-101e standards. The model is not a hypothesis. It is running.
